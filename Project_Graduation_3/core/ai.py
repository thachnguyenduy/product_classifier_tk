...
class AIEngine:
    ...
    def _decode_yolo_output(self, output, img_w, img_h):
        ...
        try:
            ...
            boxes = out_np[:, :4]
            class_scores = out_np[:, 4:4+self.num_classes]
            
            # Apply sigmoid if needed (check range)
            if np.max(class_scores) > 5.0 or np.min(class_scores) < -5.0:
            ...
            for idx in valid_indices:
                x_center = float(boxes[idx, 0]) * scale_x
                y_center = float(boxes[idx, 1]) * scale_y
                width = float(boxes[idx, 2]) * scale_x
                height = float(boxes[idx, 3]) * scale_y
